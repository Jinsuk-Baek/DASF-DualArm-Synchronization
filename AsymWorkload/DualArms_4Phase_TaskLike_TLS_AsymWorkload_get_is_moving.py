...
# -----------------------------
CART_SPEED = 80           # mm/s (nominal speed, used by Arm-A)
CART_ACC = 800            # mm/s^2
JOINT_SPEED = 20          # deg/s
JOINT_ACC = 200           # deg/s^2
WAIT = False             # Cartesian moves are dispatched non-blocking; completion is polled via get_is_moving()
...
MLS_DELAY_MS = 0.0        # Set non-zero only if you want to apply manual MLS offset

# -----------------------------
# Asymmetric workload (Comment 6 stress condition)
# -----------------------------
# Arm-B is given a sustained speed handicap relative to Arm-A for every
# move in the demo, emulating an unequal arm workload / persistent
# execution disturbance. This is a FIXED, constant scale factor applied
# throughout the run (not a per-cycle random perturbation), so that the
# resulting dispatch/finish gaps are directly attributable to the
# synchronization mechanism (TLS here; TLS+RT-MLS in the companion script)
# rather than to injected randomness.
ARM_B_HANDICAP_SCALE = 0.80   # Arm-B commanded speed = CART_SPEED * this factor

# -----------------------------
# Demo timing (cycle-based, not duration-based, for reproducibility)
# -----------------------------
NUM_CYCLES = 30
PHASE_HOLD_SEC = 0.20
...
# CSV logging
# -----------------------------
CSV_PATH = "dual_arm_tls_asym_workload_log.csv"
...
class SyncLogEntry:
    ...
    label: str
    mls_delay_ms: float
    arm_a_speed_scale: float
    arm_b_speed_scale: float
...
class RobotArm:
    ...
    def is_moving(self) -> bool:
        """Return the controller's dedicated motion-status flag."""
        ret = self.arm.get_is_moving()
        if isinstance(ret, (tuple, list)):
            if len(ret) < 2:
                raise RuntimeError(f"[{self.name}] Invalid get_is_moving response: {ret}")
            if ret[0] != 0:
                raise RuntimeError(f"[{self.name}] get_is_moving failed, ret={ret[0]}")
            return bool(ret[1])
        return bool(ret)
    ...
    def wait_until_motion_complete(
        ...
        startup_grace_sec: float = 0.10,
    ) -> None:
        """Wait until get_is_moving() confirms that Cartesian motion has stopped."""
        t0 = now()
        seen_moving = False
        ...
        while True:
            ...
            if moving:
                seen_moving = True
                consecutive_stopped = 0
            elif seen_moving or elapsed >= startup_grace_sec:
                consecutive_stopped += 1
                if consecutive_stopped >= 2:
                    return

            if elapsed > timeout_sec:
                raise RuntimeError(f"[{self.name}] motion polling timeout")
...
def run_arm_motion(
    ...
    try:
        ...
        arm.wait_until_motion_complete(timeout_sec=THREAD_JOIN_TIMEOUT_SEC - 2.0)
        finish_t = now()
...
def synchronized_cartesian_move(
    ...
    print(f"\n[SYNC] {label}")
    print(f"[SYNC] Arm-A target: {[f'{v:.2f}' for v in pose_a]}  (speed={speed_a:.1f} mm/s)")
    print(f"[SYNC] Arm-B target: {[f'{v:.2f}' for v in pose_b]}  (speed={speed_b:.1f} mm/s)")
    ...
    th_a = threading.Thread(
        target=run_arm_motion,
        args=(arm_a, pose_a, barrier, output, "A", speed_a, delay_a),
        daemon=True,
    )
    th_b = threading.Thread(
        target=run_arm_motion,
        args=(arm_b, pose_b, barrier, output, "B", speed_b, delay_b),
        daemon=True,
    )
    ...
    if th_a.is_alive() or th_b.is_alive():
        ...
        log_entries.append(
            SyncLogEntry(
                ...
                arm_a_speed_scale=speed_a / CART_SPEED,
                arm_b_speed_scale=speed_b / CART_SPEED,
        ...
        raise RuntimeError(f"[SYNC] Thread join timeout during '{label}'")
    ...
    if "A" not in output or "B" not in output:
        ...
        raise RuntimeError(f"[SYNC] Missing thread output during '{label}'")
    ...
    if not res_a.success or not res_b.success:
        ...
        raise RuntimeError(f"[SYNC] Motion failed during '{label}': {error_msg}")
    ...
    print(f"[SYNC] Dispatch gap: {dispatch_gap_ms:.2f} ms")
    print(f"[SYNC] Finish gap  : {finish_gap_ms:.2f} ms")
...
def run_task_like_demo(
    ...
    print(
        f"\n[DEMO] Running {num_cycles}-cycle asymmetric-workload TLS demo "
        f"(Arm-B handicap scale = {ARM_B_HANDICAP_SCALE:.2f})..."
    )

    speed_a = CART_SPEED
    speed_b = CART_SPEED * ARM_B_HANDICAP_SCALE
    ...
    print("[DEMO] Asymmetric-workload TLS demo completed.")
    return record_id
...
def main() -> None:
    ...
    try:
        ...
        # Anchor sync move (cycle 0) — both arms at nominal speed for the
        # anchor move itself; the handicap applies only during the task pattern.
        record_id = synchronized_cartesian_move(
            arm_a,
            anchor_a,
            arm_b,
        ...
        print("\n✔ Asymmetric-workload TLS demo completed successfully.")

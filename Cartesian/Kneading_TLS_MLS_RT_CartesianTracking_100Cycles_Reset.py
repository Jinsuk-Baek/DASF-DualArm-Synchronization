...
RT_MLS_ENABLED = True
EXPERIMENT_LABEL = "TLS+RT-MLS-CARTESIAN-TRACKING"
...
VELOCITY_MODE_ENTRY_SETTLE_SEC = 0.30
VELOCITY_MODE_EXIT_SETTLE_SEC = 0.25
VELOCITY_COMMAND_RETRIES = 2
VELOCITY_RETRY_DELAY_SEC = 0.15
...
# Both arms start at 1.0; RT-MLS may adapt the scales during motion.
MLS_MIN_SCALE = 0.65
MLS_MAX_SCALE = 1.15
...
# ============================================================
# Nominal Cartesian-tracking experiment parameters
# ============================================================
NUM_CYCLES = 100
PHASE_HOLD_SEC = 0.20
TRACKING_SETTLE_SEC = 0.10
RESET_AFTER_EVERY_CYCLES = 1
RESET_SETTLE_SEC = 0.75

CSV_PATH = "kneading_tls_mls_rt_cartesian_tracking_100cycles_reset_log.csv"
...
TASK_PATTERN: List[Tuple[str, Tuple[float, float, float]]] = [
    ("Press-Forward",  (90.0,  0.0, -20.0)),
    ("Press-Right",    (90.0, 70.0, -20.0)),
    ("Press-Back",     ( 0.0, 70.0, -20.0)),
    ("Press-Left",     ( 0.0,  0.0, -20.0)),
    ("Press-Diagonal", (75.0, 55.0, -25.0)),
    ("Lift-Center",    (40.0, 30.0,  25.0)),
    ("Press-Center",   (40.0, 30.0, -25.0)),
    ("Return",         ( 0.0,  0.0,   0.0)),
]
...
class SyncLogEntry:
    ...
    arm_a_actual_x: float
    arm_a_actual_y: float
    arm_a_actual_z: float
    arm_a_tracking_error_mm: float

    arm_b_actual_x: float
    arm_b_actual_y: float
    arm_b_actual_z: float
    arm_b_tracking_error_mm: float
...
class RobotArm:
    ...
    def enter_cartesian_velocity_mode(self) -> None:
        """Enter mode 5 and verify that the controller is ready."""
        for attempt in range(VELOCITY_COMMAND_RETRIES + 1):
            self.arm.clean_error()
            self.arm.clean_warn()
            self.arm.motion_enable(True)

            ret_mode = self._ret_code(self.arm.set_mode(5))
            ret_state = self._ret_code(self.arm.set_state(0))
            time.sleep(VELOCITY_MODE_ENTRY_SETTLE_SEC)

            mode_ok = getattr(self.arm, "mode", 5) == 5
            state_ok = self.arm.state in (0, 1, 2)
            error_ok = self.arm.error_code == 0

            if ret_mode == 0 and ret_state == 0 and mode_ok and state_ok and error_ok:
                return

            if attempt < VELOCITY_COMMAND_RETRIES:
                time.sleep(VELOCITY_RETRY_DELAY_SEC)

        raise RuntimeError(
            f"[{self.name}] Failed to enter Cartesian velocity mode "
            f"(mode={getattr(self.arm, 'mode', 'unknown')}, "
            f"state={self.arm.state}, error={self.arm.error_code}, "
            f"warn={self.arm.warn_code})"
        )

    def exit_cartesian_velocity_mode(self) -> None:
        """Return to position mode after the motion thread has stopped."""
        try:
            self.arm.set_mode(0)
            self.arm.set_state(0)
            time.sleep(VELOCITY_MODE_EXIT_SETTLE_SEC)
        except Exception:
            pass
    ...
    def command_cartesian_velocity(
        ...
        """Send a mode-5 velocity command with bounded recovery retries."""
        last_ret = -1
        ...
        for attempt in range(VELOCITY_COMMAND_RETRIES + 1):
            ...
            last_ret = self._ret_code(ret)

            if last_ret == 0 and self.arm.error_code == 0:
                return 0

            if attempt >= VELOCITY_COMMAND_RETRIES:
                break

            # Controller error 23 can occur during rapid phase-to-phase
            # mode transitions. Clear it, re-enter mode 5, and retry the
            # same bounded velocity command.
            try:
                self.arm.clean_error()
                self.arm.clean_warn()
                self.arm.motion_enable(True)
        ...
        with state.lock:
            ...
            if RT_MLS_ENABLED:
                if abs(progress_gap) > MLS_PROGRESS_DEADBAND:
                    ...
                    # When synchronized, gently relax toward nominal scale.
                    state.scale_a += MLS_RELAXATION * (1.0 - state.scale_a)
                    state.scale_b += MLS_RELAXATION * (1.0 - state.scale_b)
            else:
                # TLS-only baseline: identical velocity-mode path follower,
                # but no execution-level speed compensation.
                state.scale_a = 1.0
                state.scale_b = 1.0
...
def synchronized_cartesian_move_tracking(
    arm_a: RobotArm,
    ...
    print(
        f"[{EXPERIMENT_LABEL}] Initial scale A/B: "
    ...
    # Motion threads already issue a zero-velocity command on normal
    # completion. Perform one best-effort stop, then return to position mode.
    ...
    # Ensure both controllers have completed the mode transition before
    # the next short Kneading phase starts.
    time.sleep(VELOCITY_MODE_EXIT_SETTLE_SEC)

    if thread_a.is_alive() or thread_b.is_alive():
        raise RuntimeError(
            f"[CARTESIAN-TRACKING] Thread join timeout during '{label}'"
        )

    if "A" not in output or "B" not in output:
        raise RuntimeError(
            f"[CARTESIAN-TRACKING] Missing thread output during '{label}'"
        )
    ...
    if not result_a.success or not result_b.success:
        ...
        raise RuntimeError(
            f"[CARTESIAN-TRACKING] Motion failed during '{label}': "
    ...
    # Use the same settling interval for both methods before reading the
    # measured TCP poses at the synchronization checkpoint.
    time.sleep(TRACKING_SETTLE_SEC)
    actual_pose_a = arm_a.get_pose()
    actual_pose_b = arm_b.get_pose()

    tracking_error_a = norm3(
        actual_pose_a[0] - pose_a[0],
        actual_pose_a[1] - pose_a[1],
        actual_pose_a[2] - pose_a[2],
    )
    tracking_error_b = norm3(
        actual_pose_b[0] - pose_b[0],
        actual_pose_b[1] - pose_b[1],
        actual_pose_b[2] - pose_b[2],
    )
    mean_tracking_error = 0.5 * (tracking_error_a + tracking_error_b)
    max_tracking_error = max(tracking_error_a, tracking_error_b)
    ...
    print(f"[TRACKING] Arm-A error      : {tracking_error_a:.3f} mm")
    print(f"[TRACKING] Arm-B error      : {tracking_error_b:.3f} mm")
    print(f"[TRACKING] Mean / max error : "
          f"{mean_tracking_error:.3f} / {max_tracking_error:.3f} mm")
    ...
    log_entries.append(
        SyncLogEntry(
            ...
            arm_a_actual_x=actual_pose_a[0],
            arm_a_actual_y=actual_pose_a[1],
            arm_a_actual_z=actual_pose_a[2],
            arm_a_tracking_error_mm=tracking_error_a,

            arm_b_actual_x=actual_pose_b[0],
            arm_b_actual_y=actual_pose_b[1],
            arm_b_actual_z=actual_pose_b[2],
            arm_b_tracking_error_mm=tracking_error_b,

            mean_tracking_error_mm=mean_tracking_error,
            max_tracking_error_mm=max_tracking_error,
...
def run_task_like_demo(
    ...
    print(
        f"\n[DEMO] Running {num_cycles}-cycle nominal "
        f"{EXPERIMENT_LABEL} Kneading experiment..."
    )
    print(
        f"[DEMO] Joint configuration will be reset every "
        f"{RESET_AFTER_EVERY_CYCLES} cycle(s)."
    )

    for cycle_index in range(1, num_cycles + 1):
        for phase_index, (phase_name, delta) in enumerate(TASK_PATTERN, start=1):
            pose_a = make_arm_a_pose(anchor_a, delta)
            pose_b = make_arm_b_pose(anchor_b, delta)

            try:
                record_id = synchronized_cartesian_move_tracking(
                    arm_a=arm_a,
                    pose_a=pose_a,
                    arm_b=arm_b,
                    pose_b=pose_b,
                ...
                # Preserve every successfully completed checkpoint before
                # propagating the error for diagnosis.
                save_csv(log_entries, CSV_PATH)
                print(f"[CHECKPOINT] Partial CSV saved to: {CSV_PATH}")
                raise

            # Save after every completed phase so no valid measurements are
            # lost if the controller stops later in the run.
            save_csv(log_entries, CSV_PATH)
            time.sleep(PHASE_HOLD_SEC)

        # Force the same joint configuration after every complete cycle.
        # The Cartesian Return phase brings both TCPs to the anchor, while
        # this joint-space reset prevents gradual IK branch drift toward a
        # joint limit during repeated Kneading motion.
        if cycle_index % RESET_AFTER_EVERY_CYCLES == 0 and cycle_index < num_cycles:
            print(
                f"\n[RESET] Completed cycle {cycle_index}. "
                f"Restoring fixed ready joint configuration..."
            )
            move_to_safe_ready_poses(arm_a, arm_b)
            time.sleep(RESET_SETTLE_SEC)

            # Verify that the reset returned both TCPs close to the original
            # anchors before beginning the next measured cycle.
            reset_pose_a = arm_a.get_pose()
            reset_pose_b = arm_b.get_pose()
            reset_error_a = norm3(
                reset_pose_a[0] - anchor_a[0],
                reset_pose_a[1] - anchor_a[1],
                reset_pose_a[2] - anchor_a[2],
            )
            reset_error_b = norm3(
                reset_pose_b[0] - anchor_b[0],
                reset_pose_b[1] - anchor_b[1],
                reset_pose_b[2] - anchor_b[2],
            )
            print(
                f"[RESET] Anchor error A/B: "
                f"{reset_error_a:.3f} / {reset_error_b:.3f} mm"
            )

    print(f"[DEMO] {EXPERIMENT_LABEL} experiment completed.")
    return record_id
...
def main() -> None:
    ...
    try:
        ...
        print(f"\n{EXPERIMENT_LABEL} experiment completed successfully.")

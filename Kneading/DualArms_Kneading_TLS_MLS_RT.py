"""
TLS+RT-MLS timing experiment for reproducing dispatch-gap and finish-gap
results. Uses SDK-supported Cartesian velocity control (mode 5); no
Cartesian tracking-error measurement is included.
"""
...
RT_MLS_ENABLED = True
EXPERIMENT_LABEL = "TLS+RT-MLS-TIMING"
...
# ============================================================
# Nominal timing experiment parameters
# ============================================================
NUM_CYCLES = 100
PHASE_HOLD_SEC = 0.20

CSV_PATH = "kneading_tls_mls_rt_timing_log.csv"
...
def synchronized_cartesian_move_rt(
    arm_a: RobotArm,
    ...
    if thread_a.is_alive() or thread_b.is_alive():
        raise RuntimeError(
            f"[TLS+RT-MLS-TIMING] Thread join timeout during '{label}'"
        )

    if "A" not in output or "B" not in output:
        raise RuntimeError(
            f"[TLS+RT-MLS-TIMING] Missing thread output during '{label}'"
        )
    ...
    if not result_a.success or not result_b.success:
        ...
        raise RuntimeError(
            f"[TLS+RT-MLS-TIMING] Motion failed during '{label}': "
...
def run_task_like_demo(
    ...
    for step in range(total_steps):
        ...
        record_id = synchronized_cartesian_move_rt(
            arm_a=arm_a,
            pose_a=pose_a,
            arm_b=arm_b,
            pose_b=pose_b,
...
def main() -> None:
    ...
    try:
        ...
        # The return-to-anchor motion is a safety action and is excluded from
        # the timing dataset used to reproduce the task-phase results.
        if log_entries and log_entries[-1].phase_name == "ReturnAnchor":
            log_entries.pop()

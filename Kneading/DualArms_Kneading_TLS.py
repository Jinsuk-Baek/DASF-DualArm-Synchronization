...
# CSV logging
# -----------------------------
CSV_PATH = "dual_arm_kneading_tls_log.csv"
...
def synchronized_cartesian_move(
    ...
    print(f"\n[SYNC] {label}")
    print(f"[SYNC] Arm-A target: {[f'{v:.2f}' for v in pose_a]}")
    print(f"[SYNC] Arm-B target: {[f'{v:.2f}' for v in pose_b]}")
    ...
    th_a = threading.Thread(
        ...
        args=(arm_a, pose_a, barrier, output, "A", delay_a),
        daemon=True,
    )
    th_b = threading.Thread(
        target=run_arm_motion,
        args=(arm_b, pose_b, barrier, output, "B", delay_b),
        daemon=True,
    )
...
def run_task_like_demo(
    ...
    print(f"\n[DEMO] Running Kneading synchronized demo for {duration_sec:.1f} seconds...")
    start_t = now()
    step = 0
    ...
    print("[DEMO] Kneading demo completed.")
    return record_id
...
def main() -> None:
    ...
    try:
        ...
        print("\n✔ Both arms completed the 4-phase task-like synchronized demo successfully.")

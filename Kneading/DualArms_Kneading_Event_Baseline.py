...
# -----------------------------
# Event-based synchronization parameters
# -----------------------------
THREAD_JOIN_TIMEOUT_SEC = 25.0
EVENT_LEAD_TIME_SEC = 0.010   # 10 ms lead before releasing both threads
...
# CSV logging
# -----------------------------
CSV_PATH = "dual_arm_kneading_event_baseline_log.csv"
...
def synchronized_cartesian_move_event(
    arm_a: RobotArm,
    ...
    print(f"\n[EVENT] {label}")
    print(f"[EVENT] Arm-A target: {[f'{v:.2f}' for v in pose_a]}")
    print(f"[EVENT] Arm-B target: {[f'{v:.2f}' for v in pose_b]}")
    ...
    th_a = threading.Thread(
        target=run_arm_motion_event,
        args=(arm_a, pose_a, start_event, output, "A", delay_a),
        daemon=True,
    )
    th_b = threading.Thread(
        target=run_arm_motion_event,
        args=(arm_b, pose_b, start_event, output, "B", delay_b),
        daemon=True,
    )
    ...
    time.sleep(EVENT_LEAD_TIME_SEC)
    start_event.set()
    ...
    if th_a.is_alive() or th_b.is_alive():
        ...
        raise RuntimeError(f"[EVENT] Thread join timeout during '{label}'")
    ...
    if "A" not in output or "B" not in output:
        ...
        raise RuntimeError(f"[EVENT] Missing thread output during '{label}'")
    ...
    if not res_a.success or not res_b.success:
        ...
        raise RuntimeError(f"[EVENT] Motion failed during '{label}': {error_msg}")
    ...
    print(f"[EVENT] Dispatch gap: {dispatch_gap_ms:.2f} ms")
    print(f"[EVENT] Finish gap  : {finish_gap_ms:.2f} ms")
...
def run_task_like_demo_event(
    ...
    print(f"\n[DEMO] Running 4-phase task-like Event-based synchronized Cartesian demo for {duration_sec:.1f} seconds...")
    start_t = now()
    step = 0
    ...
    while (now() - start_t) < duration_sec:
        ...
        record_id = synchronized_cartesian_move_event(
            arm_a,
            pose_a,
            arm_b,
            pose_b,
    ...
    print("[DEMO] Event-based task-like Cartesian demo completed.")
    return record_id
...
def main() -> None:
    ...
    try:
        ...
        record_id = run_task_like_demo_event(
            arm_a,
            arm_b,
            anchor_a,
            anchor_b,

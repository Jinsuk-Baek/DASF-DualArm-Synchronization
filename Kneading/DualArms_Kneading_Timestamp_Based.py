...
# -----------------------------
# Timestamp-based synchronization parameters
# -----------------------------
THREAD_JOIN_TIMEOUT_SEC = 25.0
TIMESTAMP_LEAD_TIME_SEC = 0.050   # target timestamp 50 ms in the future
MLS_DELAY_MS = 0.0            # Keep 0.0 for TLS baseline comparison

# -----------------------------
# Demo timing
# -----------------------------
DEMO_DURATION_SEC = 1200.0
PHASE_HOLD_SEC = 0.20

# -----------------------------
# CSV logging
# -----------------------------
CSV_PATH = "dual_arm_kneading_timestamp_based_log.csv"
...
# -----------------------------
# Kneading motion: both arms repeatedly move up and down simultaneously
# Simulates synchronized kneading/cutting action in cooking
...
def synchronized_cartesian_move_timestamp(
    arm_a: RobotArm,
    ...
    print(f"\n[TIME] {label}")
    print(f"[TIME] Arm-A target: {[f'{v:.2f}' for v in pose_a]}")
    print(f"[TIME] Arm-B target: {[f'{v:.2f}' for v in pose_b]}")

    output: Dict[str, ThreadResult] = {}
    target_time = now() + TIMESTAMP_LEAD_TIME_SEC

    delay_a = max(0.0, mls_delay_ms)
    delay_b = max(0.0, -mls_delay_ms)

    th_a = threading.Thread(
        target=run_arm_motion_timestamp,
        args=(arm_a, pose_a, target_time, output, "A", delay_a),
        daemon=True,
    )
    th_b = threading.Thread(
        target=run_arm_motion_timestamp,
        args=(arm_b, pose_b, target_time, output, "B", delay_b),
        daemon=True,
    )
    ...
    if th_a.is_alive() or th_b.is_alive():
        ...
        raise RuntimeError(f"[TIME] Thread join timeout during '{label}'")
    ...
    if "A" not in output or "B" not in output:
        ...
        raise RuntimeError(f"[TIME] Missing thread output during '{label}'")
    ...
    if not res_a.success or not res_b.success:
        ...
        raise RuntimeError(f"[TIME] Motion failed during '{label}': {error_msg}")
    ...
    print(f"[TIME] Dispatch gap: {dispatch_gap_ms:.2f} ms")
    print(f"[TIME] Finish gap  : {finish_gap_ms:.2f} ms")
...
def run_task_like_demo_timestamp(
    arm_a: RobotArm,
    arm_b: RobotArm,
    ...
    print(f"\n[DEMO] Running 4-phase task-like timestamp-based synchronized Cartesian demo for {duration_sec:.1f} seconds...")
    start_t = now()
    step = 0
    ...
    while (now() - start_t) < duration_sec:
        ...
        record_id = synchronized_cartesian_move_timestamp(
            arm_a,
            pose_a,
            arm_b,
            pose_b,
    ...
    print("[DEMO] Timestamp-based Cartesian demo completed.")
    return record_id
...
def main() -> None:
    ...
    try:
        ...
        record_id = run_task_like_demo_timestamp(
            arm_a,
            arm_b,
            anchor_a,
            anchor_b,
